...
J = -sum(sum(y_matrix .* log(hypothesis) + (1 - y_matrix) .* log(1 - hypothesis))) / m + (lambda / (2 * m)) * (sum(sum(Theta1(:, 2: end) .^ 2)) + sum(sum(Theta2(:, 2: end) .^ 2)));
...
for t = 1: m,
    ...
    delta_3 = (hypothesis(t, :) - y_matrix(t, :))';
